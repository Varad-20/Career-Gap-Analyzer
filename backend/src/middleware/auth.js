...
const protect = async (req, res, next) => {
    ...
    try {
        ...
        let user = await Student.findById(decoded.id).select('-password').populate('college', 'name code city');
        ...
        if (!user) user = await Coordinator.findById(decoded.id).select('-password').populate('college', 'name code city');
